defmodule AceApp.GameDataSetup do
  ...
  def setup_all_data(opts \\ []) do
    Logger.info("🚀 Starting League of Legends game data setup")
  ...
  def setup_champions(opts \\ []) do
    ...
    if not force_update do
      champion_count = Repo.aggregate(Champion, :count, :id)
      if champion_count > 0 do
        Logger.info("✅ Champions already exist (#{champion_count} found). Use force_update: true to refresh.")
        :ok
      else
